# File Name: TwoSidedTwoMeanZTest.R
...

{
  ...
  sigy <- as.double(readline(prompt = "Enter sigmay: "))
  while (sigy <= 0.0) {
    sigy <- as.double(readline(prompt = "Enter sigmay: "))
  }
  ...
  z.test(x = x, y = y, alternative = "two.sided", sigma.x = sigx, sigma.y = sigy, conf.level = CL)
}
